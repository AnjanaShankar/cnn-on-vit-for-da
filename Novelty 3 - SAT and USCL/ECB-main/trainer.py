...
def get_pseduo_label_info(confidences_all, predict, labels, base_thresh):
    confidences = torch.cat(confidences_all, dim=0)
    dynamic_thresh = base_thresh + 0.1 * torch.std(confidences)  # Adaptive adjustment
    
    masks_bool = confidences > dynamic_thresh
    masks_idx = torch.nonzero(masks_bool, as_tuple=True)[0].numpy()

    total_pl = len(masks_idx)
    correct_pl = torch.sum(predict[masks_bool] == labels[masks_bool]).item() if total_pl > 0 else -1.0
    pl_acc = correct_pl / total_pl if total_pl > 0 else -1.0
...
def train(config, G1, G2, F1, F2, dset_loaders):
    ...
    for step in range(config["adapt_iters"]):
       ...
       # Initialize adaptive thresholds
        if step == 0:
            classwise_conf_cnn = torch.ones(config["num_classes"]).to(DEVICE) * config["thresh_CNN"]
            classwise_conf_vit = torch.ones(config["num_classes"]).to(DEVICE) * config["thresh_ViT"]

        # Shared Forward + SAT + USCL Setup
        G1.eval(); G2.eval(); F1.eval(); F2.eval()
        with torch.no_grad():
            cnn_logits_u = F2(G2(inputs_target_w))
            vit_logits_u = F1(G1(inputs_target_w))

            cnn_probs_u = F.softmax(cnn_logits_u, dim=1)
            vit_probs_u = F.softmax(vit_logits_u, dim=1)

            cnn_conf_u, cnn_plabels_u = torch.max(cnn_probs_u, 1)
            vit_conf_u, vit_plabels_u = torch.max(vit_probs_u, 1)

            for cls in range(config["num_classes"]):
                cls_mask = cnn_plabels_u == cls
                if cls_mask.sum() > 0:
                    classwise_conf_cnn[cls] = (
                        config.get("sat_alpha", 0.9) * classwise_conf_cnn[cls] +
                        (1 - config.get("sat_alpha", 0.9)) * cnn_conf_u[cls_mask].mean()
                    )

                cls_mask = vit_plabels_u == cls
                if cls_mask.sum() > 0:
                    classwise_conf_vit[cls] = (
                        config.get("sat_alpha", 0.9) * classwise_conf_vit[cls] +
                        (1 - config.get("sat_alpha", 0.9)) * vit_conf_u[cls_mask].mean()
                    )

            # ⚠️ Cast to long and validate indices before indexing!
            cnn_plabels_u = cnn_plabels_u.long()
            vit_plabels_u = vit_plabels_u.long()

            assert cnn_plabels_u.min() >= 0 and cnn_plabels_u.max() < config["num_classes"], \
                f"cnn_plabels_u out of bounds: min={cnn_plabels_u.min()}, max={cnn_plabels_u.max()}"
            assert vit_plabels_u.min() >= 0 and vit_plabels_u.max() < config["num_classes"], \
                f"vit_plabels_u out of bounds: min={vit_plabels_u.min()}, max={vit_plabels_u.max()}"

            # Adaptive confidence thresholds
            pseudo_thresh_cnn = classwise_conf_cnn[cnn_plabels_u]
            pseudo_thresh_vit = classwise_conf_vit[vit_plabels_u]

            # Masks
            pseudo_mask_cnn = cnn_conf_u >= pseudo_thresh_cnn
            pseudo_mask_vit = vit_conf_u >= pseudo_thresh_vit
            unreliable_mask_cnn = ~pseudo_mask_cnn
            unreliable_mask_vit = ~pseudo_mask_vit

            # Detach logits for safe reuse
            vit_logits = vit_logits_u.detach()
            cnn_logits = cnn_logits_u.detach()

        # Free no longer needed memory
        del cnn_probs_u, vit_probs_u, cnn_logits_u, vit_logits_u
        ...
        loss_vit_to_cnn = consistency_loss(
            logits_u_s[nl:], vit_logits, threshold=pseudo_thresh_vit
        )
        loss_vit_to_cnn.backward()
        optimizer_g2.step()
        optimizer_f2.step()
        del logits_u_s, vit_logits, pseudo_thresh_vit
        torch.cuda.empty_cache()

        # CNN -> ViT
        zero_grad_all()
        cnn_logits = F2(G2(labeled_targetw_input)).detach()
        logits_u_s = F1(G1(inputs_target_str))
        loss_cnn_to_vit = consistency_loss(
            logits_u_s, cnn_logits[nl:], threshold=pseudo_thresh_cnn
        )
        loss_cnn_to_vit.backward()
        optimizer_g1.step()
        optimizer_f1.step()
        del logits_u_s, cnn_logits, pseudo_thresh_cnn
        torch.cuda.empty_cache()

        # Unreliable Sample Contrastive Loss
        MAX_UNRELIABLE = 256

        # CNN USCL
        zero_grad_all()
        unrel_feats_cnn = G2(inputs_target_w)[unreliable_mask_cnn]
        unrel_labels_cnn = cnn_plabels_u[unreliable_mask_cnn]
        if unrel_feats_cnn.size(0) > 1:
            if unrel_feats_cnn.size(0) > MAX_UNRELIABLE:
                idx = torch.randperm(unrel_feats_cnn.size(0))[:MAX_UNRELIABLE]
                unrel_feats_cnn = unrel_feats_cnn[idx]
                unrel_labels_cnn = unrel_labels_cnn[idx]
            uscl_loss_cnn = contrastive_loss(unrel_feats_cnn, unrel_labels_cnn)
            uscl_loss_cnn.backward()
            optimizer_g2.step()
        del unrel_feats_cnn, unrel_labels_cnn
        torch.cuda.empty_cache()

        # ViT USCL
        zero_grad_all()
        unrel_feats_vit = G1(inputs_target_w)[unreliable_mask_vit]
        unrel_labels_vit = vit_plabels_u[unreliable_mask_vit]
        if unrel_feats_vit.size(0) > 1:
            if unrel_feats_vit.size(0) > MAX_UNRELIABLE:
                idx = torch.randperm(unrel_feats_vit.size(0))[:MAX_UNRELIABLE]
                unrel_feats_vit = unrel_feats_vit[idx]
                unrel_labels_vit = unrel_labels_vit[idx]
            uscl_loss_vit = contrastive_loss(unrel_feats_vit, unrel_labels_vit)
            uscl_loss_vit.backward()
            optimizer_g1.step()
        del unrel_feats_vit, unrel_labels_vit
        torch.cuda.empty_cache()
        ...
        if step % 20 == 0 or step == config["adapt_iters"] - 1:
            ...
            # log_str+= "Memory: " + str(torch.cuda.memory_summary(device=DEVICE, abbreviated=True)) + "\n"
            write_logs(out_file, log_str)
