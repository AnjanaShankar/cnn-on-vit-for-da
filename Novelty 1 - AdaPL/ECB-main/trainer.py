...
def get_adaptive_threshold_from_confidences(confidences, percentile=80):
    """Calculate adaptive threshold from confidence scores."""
    return torch.quantile(confidences, percentile / 100.0).item()

def get_adaptive_threshold_from_logits(logits, percentile=80):
    """Compute threshold from model logits using softmax confidence."""
...
def eval_domain(G1, G2, F1, F2, test_loader, **kwargs):
    ...
    thresh_cnn = None  # Will be calculated adaptively
    thresh_vit = None  # Will be calculated adaptively
    ...
    if return_pseduo:
        ...
        def get_pseduo_label_info(confidences_all, predict, labels, thresh):
            ...
            confidences = torch.cat(confidences_all, dim=0)
            thresh = get_adaptive_threshold_from_confidences(confidences, percentile=80)
            masks_bool = confidences > thresh
...
def train(config, G1, G2, F1, F2, dset_loaders):
    ...
    for step in range(config["adapt_iters"]):
        ...
        # )
        adaptive_thresh_vit = get_adaptive_threshold_from_logits(vit_logits, percentile=80)
        loss_vit_to_cnn = consistency_loss(
            logits_u_s[nl:], vit_logits, threshold=adaptive_thresh_vit
        )
        ...
        # )
        adaptive_thresh_cnn = get_adaptive_threshold_from_logits(cnn_logits[nl:], percentile=80)
        loss_cnn_to_vit = consistency_loss(
            logits_u_s, cnn_logits[nl:], threshold=adaptive_thresh_cnn
        )
