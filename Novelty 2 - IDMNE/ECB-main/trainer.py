...
def dynamic_entropy_threshold(probs, percentile):
    entropy = compute_entropy(probs)
    threshold = torch.quantile(entropy, percentile / 100.0).item()
    mask = entropy < threshold
    return mask, entropy, threshold

def inter_domain_mixup(x1, y1, x2, alpha):
    """Applies Mixup between two inputs and their labels (or pseudo-labels) from same classes."""
    lam = np.random.beta(alpha, alpha)
    lam = max(lam, 1 - lam)  # Ensure sufficient mixing
    mixed_x = lam * x1 + (1 - lam) * x2
    return mixed_x, y1, lam

# def inter_domain_mixup(x1, y1, x2, y2, alpha=0.2):
#     """
#     Applies Mixup between two inputs (x1, x2) from the same class and their labels (y1, y2).
#     Ensures that only samples with the same class are mixed between the two domains.

#     Args:
#     - x1: Tensor of features from the first domain (e.g., source domain).
#     - y1: Tensor of labels for the first domain.
#     - x2: Tensor of features from the second domain (e.g., target domain).
#     - y2: Tensor of labels for the second domain.
#     - alpha: Mixup hyperparameter that controls the interpolation strength.

#     Returns:
#     - mixed_x: The mixed input features.
#     - mixed_y: The mixed labels.
#     - lam: The interpolation coefficient.
#     """
    
#     # Ensure that x1 and x2 come from the same class
#     assert len(x1) == len(x2), "The batch sizes of x1 and x2 must be the same"
    
#     # Select indices where the classes are the same
#     same_class_indices = (y1 == y2)
#     x1_same = x1[same_class_indices]
#     y1_same = y1[same_class_indices]
#     x2_same = x2[same_class_indices]
    
#     if len(x1_same) == 0:
#         raise ValueError("No samples with the same class found to mix up.")
    
#     # Randomly mix the selected samples
#     n = len(x1_same)  # Use all available same-class samples
#     lam = np.random.beta(alpha, alpha)
#     lam = max(lam, 1 - lam)  # Ensure sufficient mixing
    
#     # Perform the mixup on the features from the same class
#     mixed_x = lam * x1_same + (1 - lam) * x2_same
#     mixed_y = y1_same  # The labels from domain 1 are used, as they correspond to the mixed features
...
def train(config, G1, G2, F1, F2, dset_loaders):
    ...
    for step in range(config["adapt_iters"]):
        ...
        ### Inter domain mixup MDM ###
        # Extract CNN features for source (labeled) and target (unlabeled)
        source_feat = G2(source_w)  # [N, C]
        target_feat = G2(inputs_target_w)  # [N, C]

        # Get pseudo-labels for target
        with torch.no_grad():
            logits_target = F2(target_feat).detach()
            probs_target = torch.softmax(logits_target, dim=1)
            pseudo_conf, pseudo_targets = probs_target.max(dim=1)

        # Dynamic thresholding using entropy
        mask, entropy, dynamic_thresh = dynamic_entropy_threshold(probs_target, config["percentile"])

        if mask.sum() > 0:
            # Filter target features and their pseudo-labels using the mask
            selected_target_feat = target_feat[mask]
            selected_pseudo_targets = pseudo_targets[mask]

            # Randomly sample matching number of source samples (same class as selected target samples)
            num_mix = min(source_feat.size(0), selected_target_feat.size(0))
            idx_source = torch.randperm(source_feat.size(0))[:num_mix]
            idx_target = torch.randperm(selected_target_feat.size(0))[:num_mix]

            source_batch = source_feat[idx_source]
            target_batch = selected_target_feat[idx_target]
            source_labels = source_labeled[idx_source]
            target_labels = selected_pseudo_targets[idx_target]

            # Ensure only same-class samples are mixed (MDM)
            same_class_mask = source_labels == target_labels
            if same_class_mask.sum() > 0:
                source_batch = source_batch[same_class_mask]
                target_batch = target_batch[same_class_mask]
                source_labels = source_labels[same_class_mask]
                target_labels = target_labels[same_class_mask]
                
                # Mix features + labels for same-class samples
                mixed_feat, labels, lam = inter_domain_mixup(source_batch, source_labels, target_batch, config["alpha"])

                # Forward mixed features through classifier
                mix_logits = F2(mixed_feat)

                # Compute mixup loss
                target_onehot = F.one_hot(target_labels, num_classes=mix_logits.size(1)).float()
                source_onehot = F.one_hot(labels, num_classes=mix_logits.size(1)).float()
                mix_labels = lam * source_onehot + (1 - lam) * target_onehot
                loss_mix = -(mix_labels * F.log_softmax(mix_logits, dim=1)).sum(dim=1).mean()
        ...
        # # Dynamic thresholding using entropy
        # mask, entropy, dynamic_thresh = dynamic_entropy_threshold(probs_target, percentile=60)

        # if mask.sum() > 0:
        #     selected_target_feat = target_feat[mask]
        #     selected_pseudo_targets = pseudo_targets[mask]

        #     # Randomly sample matching number of source samples
        #     num_mix = min(source_feat.size(0), selected_target_feat.size(0))
        #     idx_source = torch.randperm(source_feat.size(0))[:num_mix]
        #     idx_target = torch.randperm(selected_target_feat.size(0))[:num_mix]
        ...
        #     # Mix features + labels
        #     mixed_feat, labels, lam = inter_domain_mixup(source_batch, source_labels, target_batch)
